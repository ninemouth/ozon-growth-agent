...
├── libs/                      # 第三方依赖库 (DOMPurify HTML 清洗 + marked Markdown 渲染)
├── modules/                   # 模块化后台业务逻辑
│   ├── llmClient.js           # 大模型 API 调用 (支持文本/视觉模型配置、图片编辑、指数退避重试、SSE 流式解析)
│   ├── toolRegistry.js        # 自动化工具箱注册 (含系统页面脚本注入安全校验、以图搜图与搜索校准)
│   └── agentLoop.js           # Agent 自主思考-动作-反思状态机循环 (含智能 JSON 修复与供应链视觉 Critic)
└── skills/                    # 专家 Skill 提示词目录 (可根据业务场景自定义)
    ...
    ├── tiktok_shop_fastmoss_analyzer.skill.md # [New] TikTok Shop 爆品挖掘与达人带货审计专家 (Vision & FastMoss)
...
前端渲染采用本地打包的 `DOMPurify` 作为主清洗器，并叠加扩展内置的标签、属性和安全 CSS 白名单兜底。即便大模型从网页上抓取到恶意的 `<script>` 标签、事件属性、危险 CSS、`javascript:` 链接或嵌入式表单，页面在通过 `innerHTML` 渲染前也会自动过滤剥离，降低本地 API Key 与业务报告被 XSS 污染的风险。
...
### 🖼️ 9. 非标商品视觉寻源硬门禁 (Image-First Sourcing Gate)
针对造型、模具、花纹、材质高度决定采购匹配度的非标商品，系统现在默认走“目标主图 -> 1688/淘宝原生以图搜图 -> 列表页候选主图视觉排序 -> 详情页穿透审计”的路径。
- **自动主图获取**：优先读取当前商品页主图和候选大图；用户也可以在侧边栏粘贴图片 URL 或上传兜底图。
- **1688 图片浮层安全点击**：图片粘贴/上传后，仅点击浮层中精确的“搜索图片”文字按钮，避免误点输入框或浮层空白导致图片浮层消失。当页面隐藏真实文件输入框时，运行时只会向已识别的图片搜索浮层/上传区域派发安全的粘贴与拖放事件，不会向正文或搜索文本框兜底投递图片。
- **非标禁止文本回退**：一旦进入图片搜索路径，Critic 打回后也不会回到 1688/淘宝文本框关键词搜索；必须继续基于候选主图、截图和视觉相似度证据筛选。
- **视觉证据入报告**：国内寻源报告必须写入 `candidate_image_url`、`list_page_visual_score` 和 `visual_match_evidence`，证明候选不是只按标题关键词或价格选出。

### 🎨 10. 同 Provider 生图模型辅助搜图 (Clean Product Image Prep)
当 1688/淘宝自动框选目标图主体不完整时，可以在设置页配置同一 Provider/API Key 下的“生图模型”。Agent 可调用 `prepare_clean_product_image` 生成背景干净、主体完整的搜图参考图，再投喂给 1688/淘宝以图搜图，以提高非标商品匹配率并减少搜索轮次。若未配置或 provider 不支持图片编辑接口，系统会自动退回原始主图，不会中断流程。
...
6. 可选：如果需要提升 1688/淘宝非标商品以图搜图准确度，可在同一设置页填写“生图模型”（例如 OpenAI 的 `gpt-image-1`，或你的自定义 OpenAI-compatible 图片接口模型）。该模型复用同一个 Provider/API Key/Base URL，用于生成背景更干净、主体更完整的搜图参考图。
...
| `search_in_browser` | `query` (词), `engine` | **自动本地化翻译**并在当前浏览器中打开 Google/Amazon/Etsy/1688 进行物理搜索与跳转 |
| `prepare_clean_product_image` | `imageUrl`, `prompt` (可选) | 使用已配置的同 Provider 生图模型，把目标商品图处理成背景干净、主体完整的搜图参考图 |
| `image_search_1688` | `imageUrl` (图) | 打开 1688 搜索入口并上传目标商品图，优先走原生“以图搜款”返回真实货源候选 |
| `image_search_taobao` | `imageUrl` (图) | 打开淘宝搜索入口并上传目标商品图，用于淘系零售/代发场景的站内以图搜款 |
| `image_search_in_browser` | `imageUrl`, `tabId` | 在已打开的带图片上传控件页面中注入图片并执行站内以图搜索 |
| `agentic_web_search` | `query` (词) | **后台静默联网搜索**，仅用于物流、费率、政策、认证等纯信息核算，不改变当前 Tab 页 |
| `navigate_to` | `url` (链接) | 驱动 Tab 直接跳转到目标网页并等待渲染完成 |
| `query_market_data` | `keyword`, `platform` | 从 Helium 10 或 卖家精灵拉取真实销量大盘数据 (未配置Key时自动置灰隐藏) |
| `query_fastmoss_data` | `action`, `parameter` | 从 **FastMoss API** 拉取 TikTok 爆品、带货达人与视频 Hook 数据 (未配置Key时自动置灰隐藏) |
...
* **运行场景**：停留在海外 Ozon/Amazon 某款热销非标造型商品详情页运行，或在侧边栏直接贴入商品主图 URL/上传兜底图。系统会优先自动读取当前页商品主图，无需手动上传。
* **Harness 运行日志**：
  ```
  🚀 执行 Skill: 国内供应链源头选品与开发专家 (Auto)
  📖 加载 skill: skills/domestic_sourcing_finder.skill.md
  👁️ 视觉画像：读取目标商品主图，提取主体轮廓、材质、颜色、特殊造型和关键细节。
  ⚙️ 调用工具: image_search_1688 (自动打开 1688 并上传目标主图，精确点击“搜索图片”)
  ⚙️ 调用工具: prepare_clean_product_image (可选：当 1688 自动框选主体不完整时，生成干净主体图后再次以图搜图)
  🖼️ 列表页视觉初筛：读取 productCards 候选主图、价格、链接和屏幕坐标，先按外观/材质/结构相似度排序，再参考标题。
  🚨 1688 安全拦截：检测到验证码滑动页，自动将 1688 标签页激活至前台，等待用户滑动后后台自动恢复。
  ⚙️ 调用工具: open_new_tab (仅打开视觉排名最高、未触发材质/造型红线的详情页)
  ⚙️ 调用工具: agentic_web_search (最多 1 次，静默查询最新物流/费率/政策)
  ⚙️ 调用工具: close_tab (审计完毕后自动关闭后台 1688 标签页)
  ✅ 完成 (6 步)
  ```
* **AI 最终输出报告 (Markdown 渲染后)**：
  > ### 1. 源头厂货多维比价与合规会审表格
  > | 目标商品 | 候选主图 | 供应商名称 | 货源商品标题 | 批发价(元) | 起批量 | 视觉相似度 | 货源直达链接 | 审计建议 |
  > | :--- | :--- | :--- | :--- | :--- | :--- | :--- | :--- | :--- |
  > | 非标造型商品 | `candidate_image_url` | 源头工厂 | 同款/高度可改版货源 | 12.5 | 2件起批 | 86 / 100 | [点击跳转](https://detail.1688.com/offer/...) | 已对比候选主图的主体轮廓、材质反光、颜色和结构比例；若材质或造型偏离会一票否决，不再用标题关键词凑结果。
