...
6. `apply_page_filter` / `go_next_page`：优先用于筛选、排序和翻页；它们会返回点击前后页面证据变化，必须优先于裸坐标点击。
7. `click_by_coordinate`：仅用于 `apply_page_filter` 无法定位的筛选项、排序项等必要人工点击。禁止用它强行点击文件上传框、相机图标或图片浮层空白。
8. `agentic_web_search`：仅 1 次，且只用于物流、费率、政策、认证。
...
当前运行时不允许用 `agentic_web_search` 寻找 1688/淘宝货源线索。货源必须来自采购平台的站内图搜、站内文本搜索、列表页读取、语义筛选/翻页和详情页穿透。
